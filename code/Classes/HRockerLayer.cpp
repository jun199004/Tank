...
{
	this->isFollowRole = _isFollowRole;
	_active = false;
	_radius = aRadius;
	if (!_isFollowRole
		)
	...
	jsSprite = aJsSprite;
	jsSprite->setPosition(_centerPoint);
...
{
	jsSprite->setPosition(ccpAdd(jsSprite->getPosition(), ccpMult(ccpSub(_currentPoint, jsSprite->getPosition()), 0.5)));
...
bool HRockerLayer::TouchBegan(Touch *pTouch, Event *pEvent)
{
	...
	if (isFollowRole)
	{
		...
		jsSprite->setPosition(_currentPoint);
